...
export const HeroGridCard = ({ hero }: Props) => {
    ...
    const { isHeroFavorite, toggleFavorite } = use(FavoriteHeroContext);
    ...
    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64 ">
                ...
                {/* Universe badge */}
                <Badge className={`absolute top-3 right-3 text-x text-white ${hero.universe === 'DC' ? 'bg-blue-600' : 'bg-red-500'}`}>{hero.universe}</Badge>

                {/* Favorite button */}
                <Button size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white" 
                onClick={() => toggleFavorite(hero)}
                >
                    <Heart className={`h-4 w-4 ${isHeroFavorite(hero) ? 'fill-red-500 text-red-500' : ''}`} />
                </Button>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="text-xs text-gray-500 pt-2 border-t">Apareció por primera vez: {hero.firstAppearance}</div>
            </CardContent>
        </Card>
    )
}
